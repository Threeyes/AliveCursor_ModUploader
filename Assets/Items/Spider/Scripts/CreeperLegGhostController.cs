        ...
        , IAC_CommonSetting_CursorSizeHandler
{
    ...
    public Vector2 testWeightRange = new Vector2(0, 1);
    ...
    public async void TweenMoveAsync(bool forceUpdate = false)
    {
        ...
        var tweenExit = DOTween.To(() => CompWeight, (val) => CompWeight = val, testWeightRange.x, tweenDuration / 2).SetEase(easeExit);
        tweenExit.onComplete +=
            () =>
            {
                var tweenEnter = DOTween.To(() => CompWeight, (val) => CompWeight = val, testWeightRange.y, tweenDuration / 2).SetEase(easeEnter);
